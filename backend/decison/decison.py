...
dispatch_energy() (kept verbatim -- see the DISPATCH ENGINE section) into
the microgrid-status and 12-hour-schedule shapes. Anything about
battery/diesel telemetry or an hourly weather *forecast* (as opposed to
the live snapshot fetch_current gives you) doesn't exist anywhere else in
this file, so those pieces are implemented as clearly-marked placeholder
config/heuristics -- see the "MICROGRID STATE" and "FORECAST HELPERS"
sections. Replace them with your real BMS/diesel-sensor readings and a
real hourly weather forecast API when you have them; the dispatch/
endpoint wiring around them will not need to change.

*** 2026-09 fix: /predict and /schedule now agree at "hour 0" ***
Previously /predict and /schedule each called fetch_current() and
read/wrote the shared lag cache independently, with no coordination. That
meant:
  - /schedule's hour 0 was a SYNTHESIZED forecast value (clear-sky solar
    curve, persisted wind_speed) computed from whatever the cache
    happened to contain at that instant -- not the same live reading or
    the same lag history /predict would have used a moment earlier or
    later. Calling both endpoints back-to-back could (and did) produce
    different numbers for what should be the same instant.
  - .grid_state.json could silently persist stale battery/diesel numbers
    saved under an older, differently-scaled GRID_CONFIG (e.g. a
    5.4 kWh-battery mock run), permanently starving the schedule's
    battery logic even after GRID_CONFIG was corrected to real values.

Fix: _run_step() is now the single place that turns (raw weather, lag
history) into (solar_pred, wind_pred). Both predict_power() and
build_schedule() call it. build_schedule()'s hour 0 calls it with the
SAME raw = fetch_current(lat, lon) reading and the SAME cached lag
history /predict would use -- so /predict and /schedule agree at hour 0
whenever nothing else has advanced the cache in between. Hours 1..N-1
still use the synthetic forecast heuristics (no real hourly forecast API
exists here), autoregressing off that same hour-0 step.
GRID_STATE is now version-stamped so a config-scale change resets stale
persisted state instead of silently keeping it.
...
def _run_step(raw, solar_hist, wind_hist, device=None):
    """The ONE place that turns (raw weather, lag history) into
    (solar_pred, wind_pred). Both predict_power() and build_schedule()'s
    hour-0 call this with the same inputs, so 'what does the model say
    right now' means exactly the same thing in both endpoints. Returns
    (solar_pred, wind_pred, ood_warnings); does NOT mutate history or the
    cache -- callers own that."""
    device = device or _DEVICE

    ood_warnings = check_in_distribution(raw)

    solar_vec = build_solar_feature_vec(raw, _solar_ckpt, solar_hist)
    solar_pred = _predict(_solar_model, _solar_ckpt, solar_vec, device)
    solar_pred = apply_solar_physics_floor(solar_pred, raw)

    wind_vec = build_wind_feature_vec(raw, _wind_ckpt, wind_hist)
    wind_pred = _predict(_wind_model, _wind_ckpt, wind_vec, device)
    wind_pred = apply_wind_cutout_ceiling(wind_pred, raw)

    return solar_pred, wind_pred, ood_warnings
...
def predict_power(lat, lon, device=None, estimate_uncertainty=False):
    ...
    solar_pred, wind_pred, ood_warnings = _run_step(
        raw, state["solar_power_history"], state["wind_power_history"], device
    )
...
# ==========================================================================
# DISPATCH ENGINE -- kept exactly as specified. Priority for normal mode is
...
#
# GRID_CONFIG_VERSION -- bump this any time you change the *scale* of
# battery/diesel numbers below. .grid_state.json stores the version it
# was written under; if it doesn't match, the persisted file is treated
# as stale and reset to defaults instead of silently kept. This is what
# was previously causing battery_kwh to sit at 0.0 forever after
# GRID_CONFIG's capacity was rescaled from a 5.4 kWh mock value to the
# real 200 kWh spec -- the old, already-drained 0.0 kept getting loaded
# back in under the new config.
# ==========================================================================
GRID_STATE_PATH = os.path.join(_HERE, ".grid_state.json")
GRID_CONFIG_VERSION = 2
...
def _load_grid_state():
    ...
    try:
        ...
        if state.get("_config_version") != GRID_CONFIG_VERSION:
            # Written under a different GRID_CONFIG scale -- e.g. an old
            # 5.4 kWh-capacity run whose battery_kwh has already drained
            # to 0.0 and would otherwise get reloaded forever. Reset.
            return _default_grid_state()
...
def _save_grid_state(state):
    state["_config_version"] = GRID_CONFIG_VERSION
...
# ==========================================================================
# FORECAST HELPERS (for /schedule, hours 1..N-1 only)
#
...
# source available here. Hour 0 of /schedule is NEVER synthesized (see
# build_schedule): it's the actual live reading, run through the same
# _run_step() /predict uses. Only hours 1..N-1 use the heuristics below:
...
def _forecast_future_inputs(raw_now, hours):
    """Synthetic per-hour weather forecast for FUTURE hours only
    (h=1..hours-1). Hour 0 ("right now") is handled separately in
    build_schedule() using the real raw_now reading directly -- it is
    never synthesized, so /predict and /schedule agree at h=0."""
...
def _dispatch_hour(h, solar_kw, wind_kw, demand_kw, battery_kwh, is_calamity):
    """Shared per-hour bookkeeping: run dispatch_energy, build the
    schedule-row dict, and return (row, new_battery_kwh)."""
    ...
    new_battery_kwh = dispatch["new_battery_charge"]
    ...
    row = {
        ...
        "battery_soc_after": round(new_battery_kwh, 3),
        "reason": _explain_dispatch(dispatch, is_calamity),
    }
    return row, new_battery_kwh


def build_schedule(lat, lon, is_calamity=False, hours=12):
    """Runs the 12-hour dispatch loop and persists the resulting lag
    history and battery/diesel state.

    Hour 0 is the live "right now" reading, run through the exact same
    _run_step() /predict uses -- calling /predict and /schedule back to
    back (with nothing else touching this location's cache in between)
    will agree at hour 0. Hours 1..N-1 forecast forward from there using
    the synthetic heuristics in _forecast_future_inputs()."""
    ...
    # ---- Hour 0: identical to /predict -- real raw_now, real cached
    # history, same _run_step(). Not synthesized. ----
    now_hour = datetime.now().hour
    solar_pred, wind_pred, _ = _run_step(raw_now, solar_hist, wind_hist)
    solar_hist = (solar_hist + [solar_pred])[-max_solar_lag:]
    wind_hist = (wind_hist + [wind_pred])[-max_wind_lag:]

    solar_kw0 = max(solar_pred, 0.0)
    wind_kw0 = max(wind_pred, 0.0)
    demand_kw0 = _forecast_demand_kw(now_hour)
    row0, battery_kwh = _dispatch_hour(0, solar_kw0, wind_kw0, demand_kw0, battery_kwh, is_calamity)
    schedule.append(row0)

    # ---- Hours 1..N-1: synthetic forward forecast, autoregressing off
    # the hour-0 step above. ----
    for h, hour_info in enumerate(_forecast_future_inputs(raw_now, hours=hours), start=1):
        ...
        row, battery_kwh = _dispatch_hour(h, solar_kw, wind_kw, demand_kw, battery_kwh, is_calamity)
        schedule.append(row)
