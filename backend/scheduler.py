...
def build_schedule():
    """Forward-looking 12-hour schedule for the frontend's /schedule
    endpoint. Does not mutate real AppState -- it forks the current
    battery/lag state and simulates forward against the hourly forecast."""
    try:
        hourly = weather.fetch_hourly(st.LAT, st.LON, hours=st.SCHEDULE_HOURS)
    except Exception:
        return []

    battery_charge = st.state.battery_charge_kwh
    power_history, pow_out_lag = st.state.get_lag_state()
    severity = st.state.severity
    max_lag = max(getattr(st.state._solar_model, "lags", [1, 2, 3]))

    now_hour = datetime.now(timezone.utc).hour
    schedule = []
    for i, raw in enumerate(hourly):
        hour_utc = (now_hour + i) % 24
        cycle = _run_one_cycle(
            raw, st.state._solar_model, st.state._wind_model,
            power_history, pow_out_lag,
            battery_charge, severity, hour_utc,
        )
        d = cycle["dispatch"]
        schedule.append({
            "hour": i,
            "diesel_kw": round(d["diesel_used"], 2),
            "solar_kw": round(cycle["solar_pred"], 2),
            "wind_kw": round(cycle["wind_pred"], 2),
            "battery_kw": round(d["battery_used"], 2),
            "demand_kw": cycle["demand_kw"],
            "battery_soc_after": round(d["new_battery_charge"], 2),
            "reason": _reason_for(cycle, severity),
        })

        # Chain forward: this hour's outputs become next hour's state.
        battery_charge = d["new_battery_charge"]
        power_history = (power_history + [cycle["solar_pred"]])[-max_lag:]
        pow_out_lag = cycle["wind_pred"]
